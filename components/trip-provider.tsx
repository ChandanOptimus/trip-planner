...
type TripContext = {
  ...
  save: (
    collection: CollectionName | "trip",
...
export function TripProvider({ children }: { children: React.ReactNode }) {
  ...
  const save: TripContext["save"] = async (collection, method, body) => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Unable to save your change.",
      );
      return false;
    }
  };
  ...
  return (
    <Context.Provider value={{ data, loading, error, refresh, save }}>
      {children}
    </Context.Provider>
  );
}
