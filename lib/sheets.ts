...
const sheetsByCollection: Record<
  CollectionName,
  { tab: string; headers: string[] }
> = {
  itinerary: {
    tab: "Itinerary",
    headers: [
  ...
  stops: {
    tab: "Stops",
    headers: ["id", "day", "type", "location", "notes", "sortOrder"],
  },

  packing: {
    tab: "Packing",
    headers: ["id", "label", "category", "packed", "sortOrder"],
  },

  expenses: {
    tab: "Expenses",
    headers: ["id", "date", "label", "amount", "category", "notes"],
  },

  ridePrep: {
    tab: "Ride Prep",
    headers: ["id", "label", "checked", "category", "sortOrder"],
  },

  notes: {
    tab: "Notes",
    headers: ["id", "title", "body", "sortOrder"],
  },
};
...
function configuration() {
  ...
  if (!spreadsheetId || !clientEmail || !privateKey)
    throw new Error(
      "Google Sheets is not configured. Add GOOGLE_SHEET_ID, GOOGLE_SERVICE_ACCOUNT_EMAIL, and GOOGLE_PRIVATE_KEY.",
    );
...
export async function getTripData(): Promise<TripData> {
  ...
  const ranges = [
    "Trip!A:B",
    ...Object.values(sheetsByCollection).map(({ tab }) => `'${tab}'!A:Z`),
  ];
  ...
  const trip: TripSettings = Object.fromEntries(
    (results[0]?.values ?? [])
      .slice(1)
      .filter((row) => row[0])
  ...
  const collections = (
    Object.keys(sheetsByCollection) as CollectionName[]
  ).reduce(
    (all, key, index) => {
      all[key] = valuesToRows(
        results[index + 1]?.values as string[][],
        sheetsByCollection[key].headers,
  ...
  return {
    trip,
    itinerary: collections.itinerary as TripData["itinerary"],
    stops: collections.stops as TripData["stops"],
    packing: collections.packing as TripData["packing"],
    expenses: collections.expenses as TripData["expenses"],
    ridePrep: collections.ridePrep as TripData["ridePrep"],
    notes: collections.notes as TripData["notes"],
  };
}
...
export async function updateRow(
  ...
  const row = config.headers.map(
    (header, index) => input[header] ?? String(rows[rowIndex][index] ?? ""),
  );
  const rowNumber = rowIndex + 1;
  await api.spreadsheets.values.update({
    spreadsheetId,
    range: `'${config.tab}'!A${rowNumber}:${columnLetter(config.headers.length)}${rowNumber}`,
    valueInputOption: "USER_ENTERED",
    requestBody: { values: [row] },
  });
}
...
export async function deleteRow(collection: CollectionName, id: string) {
  ...
  const rowIndex = (existing.data.values ?? []).findIndex(
    (row, index) => index > 0 && String(row[0]) === id,
  );
  if (rowIndex < 0) throw new Error("That item no longer exists in the sheet.");
...
export async function updateTrip(values: TripSettings) {
  ...
  const existingRows = new Map(
    existing.slice(1).map((row, index) => [String(row[0]), index + 2]),
  );
